/*
 * Program: FXMLAddProductController.java
 * Author: Davis Nguyen
 * Date: 8/22/2019 created 
 * Updated: 8/29/2019 added basic functionality to textfields and buttons
 ...
 *          9/6/2019 cleaned up code, added parts cost vs product cost check
 *
 */
...
public class FXMLAddProductController implements Initializable {
    ...
    //create and instantiate new product;
        ...
        try {
            if (nameTF.getText().isEmpty() || stockTF.getText().isEmpty() || priceTF.getText().isEmpty()
                ...
                if (Double.parseDouble(priceTF.getText()) < partsCost) {
                    ...
                    //if there is no products list, set id to 1
                    if (Inventory.getAllProducts().isEmpty()) {
                        product.setId(1);
                    } else {
                        //if there is a products list, search though the products list's id and grab the largest number
                        //set new product id to largest number + 1
                        int temp;
                        for (int i = 0; i < Inventory.getAllProducts().size(); i++) {
                            temp = Inventory.getAllProducts().get(0).getId();
                            if (Inventory.getAllProducts().get(i).getId() > temp) {
                                temp = Inventory.getAllProducts().get(i).getId();
                            }
                            product.setId(temp + 1);
                        }
                    }
                    //add data grabed from textfields into product
                    ...
                    //clear all parts associated with the product
                    product.getAllAssociatedParts().clear();

                    //for every instance of parts in addedPartsTV, add the parts to product
                    for (Part adding : addedPartsTV.getItems()) {
                        product.addAssociatedPart(adding);
                    }
